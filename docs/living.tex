...
Blockscope is a client-side Fabric 1.19.4 mod that records gameplay by running alongside ReplayMod as a companion recorder. No Minecraft server modifications are required. Three components run simultaneously on the player's machine:
...
\begin{itemize}
  ...
  \item \textbf{Hopper.} A FastAPI (Python) server receives streamed tick data, video segments, inputs, and the \texttt{.mcpr} over HTTP and writes them to a per-session folder. Currently runs locally on the recording machine; intended for deployment to a dedicated server for multi-player collection.
\end{itemize}

\textbf{Status (2026-05-05):} The full pipeline is end-to-end functional: sessions produce complete output (video, tick data, \texttt{.mcpr}, inputs, frame mapping), post-processing to per-tick \texttt{.npz} files has been validated on real session data, and Hopper is running locally on the recording machine. Stress testing under deliberate multi-player breakage attempts (concurrent sessions, abrupt disconnects, edge-case tick scenarios) has not yet been conducted; deployment to a dedicated server and data collection at scale begins once this is complete.
...
\textbf{Data collection environments:}

\begin{itemize}
  \item \textbf{Hermitcraft-style servers.} High-skill, late-game, diverse play. Covers complex builds, redstone, large-scale resource gathering. VPT contractor data is almost exclusively early-game survival; this directly addresses that gap.
  \item \textbf{Random generated worlds.} Systematically varies biomes, terrain types, and block distributions. Combined with the random block worlds used for the block tokenizer.
\end{itemize}
...
Use cases: action imitation pretraining (when action head is added); visual pretraining signal for SigLIP fine-tuning; background distribution for inventory bag-of-items prediction.
